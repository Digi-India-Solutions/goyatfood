...
const AllOrder = () => {
    ...
    const fetchOrders = async () => {
        try {
            const response = await axios.get('http://localhost:12006/api/order-data');
    ...
    const handleDelete = async (orderId) => {
        try {
            ...
            if (result.isConfirmed) {
                const response = await axios.delete(`http://localhost:12006/api/delete-order-data/${orderId}`);
                if (response.status === 200) {
