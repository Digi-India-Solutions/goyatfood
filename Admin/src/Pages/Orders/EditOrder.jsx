    ...
    useEffect(() => {
        const fetchTaxDetails = async () => {
            ...
            for (const product of order.products) {
                try {
                    const response = await axios.get(`https://goyattrading.shop/api/single-product/${product.productId}`);
...
const EditOrder = () => {
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            try {
                const response = await axios.get(`https://goyattrading.shop/api/single-order-data/${orderId}`);
    ...
    const handleUpdate = async () => {
        try {
            const response = await axios.put(`https://goyattrading.shop/api/update-order/${orderId}`, {
                orderStatus,
                paymentStatus,
            });
            if (response.data.success) {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await axios.post('https://goyattrading.shop/api/login-via-shiprocket', payload, {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('https://goyattrading.shop/api/shiped-order-shiprocket', orderData, {
                headers: { 'Content-Type': 'application/json' }
            });
            console.log(response)
            if (response.status === 200) {
